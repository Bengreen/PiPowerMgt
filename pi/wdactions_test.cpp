...
int main()
{
    ...
    std::cout << "object is " << sizeof(message_t) << endl;

    std::cout << "action is " << sizeof(action_t) << endl;
    std::cout << "task is " << sizeof(task_t) << endl;
    std::cout << "uint16 is " << sizeof(uint16_t) << endl;
    std::cout << "monkey is " << sizeof(monkey_t) << endl;

    char *ben2 = (char *)&message;

    for (int index = 0; index < sizeof(message_t); ++index)
    {
        std::cout << "[" << index << "] = " << (int)ben2[index] << endl;
    }

    arduino = wiringPiI2CSetup(watchdogAddr);

    std::cout << "Init result: " << arduino << endl;
    {
        int numSent = write(arduino, &message, sizeof(message_t));
        std::cout << "Send " << numSent << endl;
    }
}
